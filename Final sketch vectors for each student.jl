...
begin
	a_df = CSV.read("student_answers_W2023.csv", DataFrame) #answer dataframe
...
function build_sy_dataset(a_df, s_df)
    sy_data_2 = DataFrame(uid = Int64[], y_vector = Vector{Float64}[])

	#A helper function to update the y vector for a particular student, it takes the filtered answer dataset for that student, qid, sketch dataset and initial y vector as inputs
    function update_sy_vector(student_answers, qid, sketch_data, sy_data, student, current_sketch_y_vector = zeros(22))

			#for 2nd Q, defining the end_timestamp 2 and using it to filter the animation data
            ...
            animation_data = filter(row ->(row.timestamp <= end_timestamp_2), sketch_data)
		...
		#pushing the uid and fianl sketch vectors in the dataframe
...
begin
	#building the dataset containing the final y-vectors for each student
	sy_data_2 = build_sy_dataset(
		a_df, 
		s_df)
end
